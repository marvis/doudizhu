...
#define MAX_COUNT					19									//最大数目
#define FULL_COUNT					51									//全牌数目
...
#define CT_SHEN_BOMBCARD            13                                  //深水炸弹
#define CT_SHEN_MISSILE_CARD        14                                  //深水火箭
...
protected:
	...
	static const BYTE				m_cbHpCardData[FULL_COUNT];			//扑克数据
...
public:
	...
	bool IsHaveChangeThree(const BYTE cbCardData[],BYTE cbCardCount);
	//得到听用3在哪个位置
	BYTE GetWeiZhiChangeThree( const BYTE cbCardData[], BYTE cbCardCount);
	//没有听用的牌型 add by ljl 2010-10-18
	BYTE GetNoShanThreeCardType(const BYTE cbCardData[], BYTE cbCardCount);
	//有听用的牌型
	bool GetShanCardType( const BYTE cbCardData[], BYTE cbCardCount,BYTE byType[], BYTE &byCountType,BYTE byThreeCardData[] );

	//查找某张牌在哪个位置
	int GetCardCueID(const BYTE cbCardData[],BYTE  cbCardCount,BYTE byFindData);
