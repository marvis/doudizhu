...
//扑克数据
const BYTE	CGameLogic::m_cbHpCardData[FULL_COUNT]=
{
...
{
	if ( IsHaveChangeThree( cbCardData,cbCardCount ) )
	{
		BYTE byTempCard[18] = {0};
		CopyMemory(byTempCard,cbCardData,sizeof(BYTE)*cbCardCount);
		SortCardList(byTempCard,cbCardCount,ST_ORDER);
		if ( cbCardCount == 5 )                 //判断是否深水炸弹或者深水火箭
		{
			BYTE byCountCard[18] = {0};
			for ( int i=0; i<cbCardCount; i++ )
			{
				byCountCard[GetCardLogicValue(byTempCard[i])]++;
			}
			for ( int i=0; i<18; i++ )
			{
				if ( 4 == byCountCard[i] )
				{
					return CT_SHEN_BOMBCARD;
				}
			}
		}
		if ( cbCardCount == 3 )
		{
			if ( byTempCard[0] == 0x4F && byTempCard[1] == 0x4e && byTempCard[2] == 0x03 )
			{
				return CT_SHEN_MISSILE_CARD;
			}
		}
	}

	if ( !IsHaveChangeThree(cbCardData,cbCardCount) || 1 == cbCardCount )
	{
		return GetNoShanThreeCardType(cbCardData,cbCardCount);
	}
	BYTE byCardType[14] ={0};
	BYTE byCountCardType;
	BYTE byCardThreeData[18] = {0};
	if ( GetShanCardType(cbCardData,cbCardCount,byCardType,byCountCardType,byCardThreeData) )
	{
		return byCardType[0];
	}
	return CT_ERROR;
...
//没有听用的牌型 add by ljl 2010-10-18
BYTE CGameLogic::GetNoShanThreeCardType(const BYTE cbCardData[], BYTE cbCardCount)
{
	//简单牌型
	switch (cbCardCount)
	{
	...
	if (AnalyseResult.cbThreeCount>0)
	{
		...
		if(AnalyseResult.cbThreeCount>1 && (AnalyseResult.cbThreeCount*3==cbCardCount ||
			AnalyseResult.cbThreeCount*4==cbCardCount || 
...
//是否有听用3 add by ljl 2010-10-18
bool CGameLogic::IsHaveChangeThree(const BYTE cbCardData[],BYTE cbCardCount)
{
	for ( int i=0; i<cbCardCount; i++ )
	{
		if ( 3 == GetCardLogicValue(cbCardData[i]) )
		{
			return true;
		}
	}
...
//add by ljl 2010-10-18
//得到听用3在哪个位置,在保证有听用三的情况下使用，方便变牌
BYTE CGameLogic::GetWeiZhiChangeThree( const BYTE cbCardData[], BYTE cbCardCount)
{
	for ( int i=0; i<cbCardCount; i++ )
	{
		if ( 3 == GetCardLogicValue(cbCardData[i]) )
...
//有听用的牌型
bool CGameLogic::GetShanCardType( const BYTE cbCardData[], BYTE cbCardCount,BYTE byType[], BYTE &byCountType ,BYTE byThreeCardData[] )
{
	int iGetChangThree = GetWeiZhiChangeThree(cbCardData,cbCardCount);
	if ( 255 == iGetChangThree )
	{
		return CT_ERROR;
	}
	byCountType = 0;
	BYTE byCardTypeSham = CT_ERROR;
	BYTE cbTempCardData[16] = {0};
	for ( int i=37; i<=48; i++ )
	{
		::CopyMemory(cbTempCardData,cbCardData,sizeof(BYTE)*cbCardCount);
		cbTempCardData[iGetChangThree] = m_cbCardData[i];
		byCardTypeSham =  GetNoShanThreeCardType(cbTempCardData,cbCardCount);
		
		if ( byCardTypeSham != CT_ERROR )
		{
			byThreeCardData[byCountType] = m_cbCardData[i];
			byType[byCountType++] = byCardTypeSham;
		}
	}
	if ( byCountType >= 1 )
	{
...
void CGameLogic::GetGoodCardData(BYTE cbGoodCardData[NORMAL_COUNT])
{
	...
	srand(GetTickCount()+dwRandCount++);
	//混乱扑克
...
void CGameLogic::RandCardList(BYTE cbCardBuffer[], BYTE cbBufferCount)
{
	...
	CopyMemory(cbCardData,m_cbHpCardData,sizeof(m_cbCardData));

	static long int dwRandCount=0L;
	srand(GetTickCount()+dwRandCount++);

	BYTE bBombNum[GAME_PLAYER];
	ZeroMemory(bBombNum,sizeof(bBombNum));
	bBombNum[0]=rand()%3+1;
	bBombNum[1]=rand()%3+1;
	bBombNum[2]=rand()%3+1;
	BYTE cbRandCount=0,cbPosition=0;
	BYTE cbBombNum=0;
	BYTE cbNum=0;
	for(BYTE n=0; n<GAME_PLAYER; n++)
	{
		cbRandCount=0;
		for(BYTE i=0; i<bBombNum[n]; i++)
		{
			cbPosition=rand()%(13-cbBombNum);
			cbBombNum++;
			for(BYTE j=0; j<4; j++)
			{
				cbNum++;
				cbCardBuffer[(cbRandCount++)+n*17]=cbCardData[cbPosition*4+j];
				cbCardData[cbPosition*4+j]=cbCardData[cbBufferCount-cbNum-2];//-2 排除大小王
			}
		}
	}

	cbCardData[cbBufferCount-cbNum-1]=cbCardData[cbBufferCount-1];//加进大小王
	cbCardData[cbBufferCount-cbNum-2]=cbCardData[cbBufferCount-2];
	for(BYTE i=0; i<GAME_PLAYER; i++)
	{
		BYTE cbCardNum=NORMAL_COUNT*(i+1);
		if(i==GAME_PLAYER-1)
			cbCardNum+=3;

		for(BYTE j=bBombNum[i]*4+NORMAL_COUNT*i; j<cbCardNum; j++)
		{
			cbPosition=rand()%(cbBufferCount-cbNum);
			cbCardBuffer[j]=cbCardData[cbPosition];
			cbNum++;
			cbCardData[cbPosition]=cbCardData[cbBufferCount-cbNum];			
		}
	}
#else
	//混乱准备
	...
	srand(GetTickCount()+dwRandCount++);
	//混乱扑克
	BYTE cbRandCount=0,cbPosition=0/*,id=0*/;
	do
	{
		cbPosition=rand()%(cbBufferCount-cbRandCount);
		cbCardBuffer[cbRandCount++]=cbCardData[cbPosition];
		cbCardData[cbPosition]=cbCardData[cbBufferCount-cbRandCount];
		/*cbCardData[cbPosition]=cbCardData[id/4+1+(id%4)*13];
		id++;*/
	} while (cbRandCount<cbBufferCount);
#endif
	return;
...
bool CGameLogic::CompareCard(const BYTE cbFirstCard[], const BYTE cbNextCard[], BYTE cbFirstCount, BYTE cbNextCount)
{
	...
	if (cbNextType==CT_SHEN_MISSILE_CARD) return true;
	
	if (cbFirstType==CT_SHEN_MISSILE_CARD)
	{
		return false ;
	}
	...
	if ((cbFirstType!=CT_SHEN_BOMBCARD)&&(cbNextType==CT_SHEN_BOMBCARD)) return true;
	
	if ((cbFirstType==CT_SHEN_BOMBCARD)&&(cbNextType!=CT_SHEN_BOMBCARD))
	{
		return false;
	}

	if ((cbFirstType!=CT_MISSILE_CARD) && (cbNextType==CT_MISSILE_CARD)) return true;

	if ((cbFirstType==CT_MISSILE_CARD)&&(cbNextType!=CT_MISSILE_CARD))
	{
		return false;
	}
...
//查找某张牌在哪个位置
int CGameLogic::GetCardCueID(const BYTE cbCardData[],BYTE  cbCardCount,BYTE byFindData)
{
	for ( int i=0; i<cbCardCount; i++ )
	{
		if ( cbCardData[i] == byFindData )
		{
			return i;
		}
	}
...
void CGameLogic::GetAllLineCard(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE cbLineCardData[], BYTE &cbLineCardCount)
{
	...
	while (cbLeftCardCount>=5 && bFindSingleLine)
	{
		...
		if(cbSingleLineCount>=5)
		{
			if(!RemoveCard(cbSingleLineCard, cbSingleLineCount, cbTmpCard, cbLeftCardCount)) return;
...
void CGameLogic::AnalyseOutCardType(BYTE const cbHandCardData[], BYTE const cbHandCardCount, tagOutCardTypeResult CardTypeResult[12+1])
{
	...
	//单连类型
	{
		...
		while (cbLeftCardCount>=5 && bFindSingleLine)
		{
			...
			BYTE cbTempCard[20]={0};
			CopyMemory(cbTempCard,cbSingleLineCard,sizeof(BYTE)*cbSingleLineCount);
			SortCardList(cbTempCard, cbSingleLineCount, ST_ORDER);
			//保存数据
			if(cbSingleLineCount>=5 && GetCardType(cbTempCard,cbSingleLineCount)==CT_SINGLE_LINE)
			{
				BYTE Index ;
				//所有连牌
				...
				if(RemoveCard(cbSingleLineCard, cbSingleLineCount, cbTmpCardData, cbLeftCardCount)==true)
				{
					cbLeftCardCount -= cbSingleLineCount ;
					bFindSingleLine = true ;
				}
	...
	//对连类型
	{
		...
		while (cbLeftCardCount>=6 && bFindDoubleLine)
		{
			...
			BYTE cbTempCard[20]={0};
			CopyMemory(cbTempCard,cbDoubleLineCard,sizeof(BYTE)*cbDoubleLineCount);
			SortCardList(cbTempCard, cbDoubleLineCount, ST_ORDER);
			//保存数据
			if(cbDoubleLineCount>=6 && GetCardType(cbTempCard,cbDoubleLineCount)==CT_DOUBLE_LINE)
			{
				BYTE Index ;
				...
				if(RemoveCard(cbDoubleLineCard, cbDoubleLineCount, cbTmpCardData, cbFirstCard+cbLeftCardCount)==true)
				{
	...
	//三连类型
	{
		...
		while (cbLeftCardCount>=6 && bFindThreeLine)
		{
			...
			BYTE cbTempCard[20]={0};
			CopyMemory(cbTempCard,cbThreeLineCard,sizeof(BYTE)*cbThreeLineCount);
			SortCardList(cbTempCard, cbThreeLineCount, ST_ORDER);
			//保存数据
			if(cbThreeLineCount>=6 && GetCardType(cbTempCard,cbThreeLineCount)==CT_THREE_LINE)
			{
				BYTE Index ;
				...
				if(RemoveCard(cbThreeLineCard, cbThreeLineCount, cbTmpCardData, cbFirstCard+cbLeftCardCount) ==true)
				{
					bFindThreeLine=true ;
	...
	//三带一单
	{
...

		{
			...
			for(BYTE i=0; i<cbHandThreeCount; i+=3)
			{
				...
				{
					BYTE cbTempCard[4]={0};
					cbTempCard[0]=cbHandThreeCard[i];
					cbTempCard[1]=cbHandThreeCard[i+1];
					cbTempCard[2]=cbHandThreeCard[i+2];
					cbTempCard[3]=cbRemainCardData[j];
					if(GetCardType(cbTempCard,4)==CT_THREE_LINE_TAKE_ONE)
					{
	...
	//			//移除三连
	//			if(RemoveCard(cbThreeLineCard, cbThreeLineCardCount, cbHandThreeCard, cbLeftThreeCardCount)==true)
	//			{
	//				bFindThreeLine = true ;
	...
	//{
		...
		for(BYTE i=0; i<cbHandThreeCount; i+=3)
		{
			...
			for(BYTE j=0; j<cbAllDoubleCardCount; j+=2)
			{
				...
				cbTempCard[1]=cbHandThreeCard[i+1];
				cbTempCard[2]=cbHandThreeCard[i+2];
				cbTempCard[3]=cbAllDoubleCardData[j];
				cbTempCard[4]=cbAllDoubleCardData[j+1];
				if(GetCardType(cbTempCard,5)==CT_THREE_LINE_TAKE_TWO)
				{
...
void CGameLogic::AnalyseOutCardType(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE const cbTurnCardData[], BYTE const cbTurnCardCount,tagOutCardTypeResult CardTypeResult[12+1])
{
	...
	BYTE cbTmpCard[MAX_COUNT];
	CopyMemory(cbTmpCard, cbHandCardData, cbHandCardCount) ;
	BYTE cbTmpTurnCard[MAX_COUNT]={0};
	CopyMemory(cbTmpTurnCard, cbTurnCardData, cbTurnCardCount) ;
	SortCardList(cbTmpCard, cbHandCardCount, ST_ORDER) ;
	SortCardList(cbTmpTurnCard, cbTurnCardCount, ST_ORDER) ;

	BYTE cbTurnCardType = GetCardType(cbTmpTurnCard, cbTurnCardCount) ;
	...
	case CT_SINGLE:				//单牌类型
		{			
			for(BYTE i=0; i<cbHandCardCount; ++i) 
				if(GetCardLogicValue(cbTmpCard[i])>GetCardLogicValue(cbTmpTurnCard[0])) 
				{
					BYTE Index = CardTypeResult[CT_SINGLE].cbCardTypeCount ;
					CardTypeResult[CT_SINGLE].cbCardType = CT_SINGLE ;
	...
	case CT_DOUBLE:				//对牌类型
		{
			...
			for (BYTE i=0;i<cbHandCardCount;i++)
			{
				...
				if(cbSameCount>=2 && GetCardLogicValue(cbTmpCard[i])>GetCardLogicValue(cbTmpTurnCard[0]))
				{
					BYTE Index = CardTypeResult[CT_DOUBLE].cbCardTypeCount ;
					CardTypeResult[CT_DOUBLE].cbCardType = CT_DOUBLE ;
	...
	case CT_THREE:				//三条类型
		{
			...
			for (BYTE i=0;i<cbHandCardCount;i++)
			{
				...
				if(cbSameCount>=3 && GetCardLogicValue(cbTmpCard[i])>GetCardLogicValue(cbTmpTurnCard[0]))
				{
					BYTE Index = CardTypeResult[CT_THREE].cbCardTypeCount ;
					CardTypeResult[CT_THREE].cbCardType = CT_THREE ;
	...
	case CT_SINGLE_LINE:		//单连类型
		{
			...
			while (cbLeftCardCount>=cbTurnCardCount && bFindSingleLine)
			{
				...
				//保存数据
				if(cbSingleLineCount>=cbTurnCardCount && GetCardLogicValue(cbSingleLineCard[0])>GetCardLogicValue(cbTmpTurnCard[0]))
				{
					BYTE Index ;
					BYTE cbStart=0 ;
					//所有连牌
					while (GetCardLogicValue(cbSingleLineCard[cbStart])>GetCardLogicValue(cbTmpTurnCard[0]) && ((cbSingleLineCount-cbStart)>=cbTurnCardCount))
					{
						Index = CardTypeResult[CT_SINGLE_LINE].cbCardTypeCount ;
						CardTypeResult[CT_SINGLE_LINE].cbCardType = CT_SINGLE_LINE ;
	...
	case CT_DOUBLE_LINE:		//对连类型
		{
			...
			while (cbLeftCardCount>=cbTurnCardCount && bFindDoubleLine)
			{
				...
				if(cbDoubleLineCount>=cbTurnCardCount)
				{
					...
					BYTE cbStart=0 ;
					//所有连牌
					while (GetCardLogicValue(cbDoubleLineCard[cbStart])>GetCardLogicValue(cbTmpTurnCard[0]) && ((cbDoubleLineCount-cbStart)>=cbTurnCardCount))
					{
						Index = CardTypeResult[CT_DOUBLE_LINE].cbCardTypeCount ;
						CardTypeResult[CT_DOUBLE_LINE].cbCardType = CT_DOUBLE_LINE ;
	...
	case CT_THREE_LINE:			//三连类型
		{
			...
			while (cbLeftCardCount>=cbTurnCardCount && bFindThreeLine)
			{
				...
				if(cbThreeLineCount>=cbTurnCardCount)
				{
					...
					BYTE cbStart=0 ;
					//所有连牌
					while (GetCardLogicValue(cbThreeLineCard[cbStart])>GetCardLogicValue(cbTmpTurnCard[0]) && ((cbThreeLineCount-cbStart)>=cbTurnCardCount))
					{
						Index = CardTypeResult[CT_THREE_LINE].cbCardTypeCount ;
						CardTypeResult[CT_THREE_LINE].cbCardType = CT_THREE_LINE ;
	...
	case CT_THREE_LINE_TAKE_ONE://三带一单
		{
			...
			//三条扑克
			GetAllThreeCard(cbTmpTurnCard, cbTurnCardCount, cbTurnThreeCard, cbTurnThreeCount) ;
	...
	case CT_FOUR_LINE_TAKE_ONE://四带两单
		{
			...
			GetAllBomCard(cbTmpTurnCard, cbTurnCardCount, cbTurnAllFourCardData, cbTurnAllFourCardCount) ;
	...
	case CT_BOMB_CARD:			//炸弹类型
		{
			...
			{
				if(GetCardLogicValue(cbAllBomCardData[i])>GetCardLogicValue(cbTmpTurnCard[0]))
				{
					Index = CardTypeResult[CT_BOMB_CARD].cbCardTypeCount ;
					CardTypeResult[CT_BOMB_CARD].cbCardType = CT_BOMB_CARD ;
...
void CGameLogic::BankerOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, tagOutCardResult & OutCardResult) 
{
	...
	if(cbHandCardCount>=2)
	{
		...
		else
		{
			...
			if(!RemoveCard(cbAllBombCardData, cbAllBombCount, cbRemainCard, cbRemainCount)) return ;
	...
	if(CanOutCard.cbCardTypeCount>0 && cbMinSingleCountInFour < MAX_COUNT)
	{
		...
		if(bFindCard)
		{
			...
			if(GetCardLogicValue(CanOutCard.cbCardData[cbCanOutIndex][0])>=16 && cbHandCardCount>=2 && cbHandCardData[0]==0x4F && cbHandCardData[1]==0x4E)
			{
				...
				//一手牌和一个炸弹
				BYTE cbRemainCardData[MAX_COUNT], cbRemainCardCount=cbHandCardCount ;
				CopyMemory(cbRemainCardData, cbHandCardData, cbHandCardCount) ;
...
//	if(m_lRecCount>50) return false ;
	...
	if (cbAllBombCount > 0)
	{
		...
		if(!RemoveCard(cbWantOutCardData, cbWantOutCardCount, cbRemainCard, cbRemainCardCount)) return false;
	...
	if(!RemoveCard(cbWantOutCardData, cbWantOutCardCount, cbAllCardData[wTestUser], cbUserCardCount[wTestUser])) return false;
...
bool CGameLogic::TestOutAllCard( WORD wTestUser, BYTE cbWantOutCardData[], BYTE cbWantOutCardCount, bool bFirstOutCard ) {
	...
	try {
		...
		if ( cbWantOutCardCount != m_cbUserCardCount[ wTestUser ] ) {
			...
			//移除第一手牌
			if(!RemoveCard( cbWantOutCardData, cbWantOutCardCount, OriginHandCardInfo.cbHandCardData, OriginHandCardInfo.cbHandCardCount )) return false;
					...
					for ( BYTE cbHandCardIndx = 0; cbHandCardIndx < pOutCardTypeResult[ cbOutCardTypeIndx ].cbCardTypeCount && ! bBreakJudge ; ++cbHandCardIndx ) {
						...
						if ( IsLargestCard( wTestUser, pOutCardTypeResult[ cbOutCardTypeIndx ].cbCardData[ cbHandCardIndx ], pOutCardTypeResult[ cbOutCardTypeIndx ].cbEachHandCardCount[ cbHandCardIndx ] ) ) {
							...
							if ( ! VerifyOutCard( wTestUser, pOutCardTypeResult[ cbOutCardTypeIndx ].cbCardData[ cbHandCardIndx ], 
								...
								//移除当前一手
								if(!RemoveCard( pOutCardTypeResult[ cbOutCardTypeIndx ].cbCardData[ cbHandCardIndx ], pOutCardTypeResult[ cbOutCardTypeIndx ].cbEachHandCardCount[ cbHandCardIndx ],
									NewHandCardInfo.cbHandCardData, NewHandCardInfo.cbHandCardCount )) return false;
