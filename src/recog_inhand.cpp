...
void RecogInhand::recog_bbox()
{
	...
	double step = 66.66;
	int ncards = 0; //(bot - top - 150)/step + 1;
	if(ncards > 1) step = (bot - 159 - top)/(double)(ncards-1);
	cerr<<"bot - top = "<<bot - top<<"  ncards = "<<ncards<<endl;

	// 18, 19, 20 cards
	if(bot - top > (1225 + 1239)/2.0)
	{
		for(int n = 18; n <= 20; n++)
		{
	...
	{
		step = 66.66;
		ncards = (int)((bot - 159 - top)/step + 1 + 0.5);
		step = (bot - 159 - top)/(double)(ncards-1);
	}
	cout<<"ncards = "<<ncards<<" step = "<<step<<endl;
