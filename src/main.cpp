...
void loadAllTemplates()
{
	...
	file = "ddz_last_red_joker.png"; map_allImgs[file] = cvLoadImage((DDZTMPLPATH + file).c_str(), 1);
	file = "ddz_last_black_joker.png"; map_allImgs[file] = cvLoadImage((DDZTMPLPATH + file).c_str(), 1);
...
vector<Card> recog_mycards(IplImage * image)
{
	...
	for(int i = 0; i < typeBBox.size(); i++)
	{
		...
		if(1)
		{
			...
			if(isImageSame_shifty2(image, rect.x, rect.y, "ddz_mycards_red_joker.png", diffval)) {cards[i].type = TYPE_RED_JOKER; cards[i].num = NUM_JOKER; goto out1;} diffvals.push_back(diffval); 
			if(isImageSame_shifty2(image, rect.x, rect.y, "ddz_mycards_black_joker.png", diffval)) {cards[i].type = TYPE_BLACK_JOKER; cards[i].num = NUM_JOKER; goto out1;} diffvals.push_back(diffval); 
...
vector<Card> recog_last_cards(IplImage * image)
{
	...
	for(int i = 0; i < typeBBox.size(); i++)
	{
		...
		if(1)
		{
			...
			if(isImageSame_shifty2(image, rect.x, rect.y, "ddz_last_red_joker.png", diffval, 10, 0)) {cards[i].type = TYPE_RED_JOKER; cards[i].num = NUM_JOKER; goto out1;} diffvals.push_back(diffval);
			if(isImageSame_shifty2(image, rect.x, rect.y, "ddz_last_black_joker.png", diffval, 10, 0)) {cards[i].type = TYPE_BLACK_JOKER; cards[i].num = NUM_JOKER; goto out1;} diffvals.push_back(diffval);
...
vector<Card> recog_played_cards(string who, IplImage * image)
{
	...
	for(int i = 0; i < typeBBox.size(); i++)
	{
		...
		if(1)
		{
			...
			if(isImageSame_shifty2(image, rect.x, rect.y, "ddz_play_red_joker.png", diffval)) {cards[i].type = TYPE_RED_JOKER; cards[i].num = NUM_JOKER; goto out1;} diffvals.push_back(diffval);
			if(isImageSame_shifty2(image, rect.x, rect.y, "ddz_play_black_joker.png", diffval)) {cards[i].type = TYPE_BLACK_JOKER; cards[i].num = NUM_JOKER; goto out1;} diffvals.push_back(diffval);
			vector<double>::iterator min_elem =  min_element(diffvals.begin(), diffvals.end());
			if(*min_elem < 60) cards[i].type = (int)(min_elem - diffvals.begin()) + TYPE_SPADE;
			else
			{
